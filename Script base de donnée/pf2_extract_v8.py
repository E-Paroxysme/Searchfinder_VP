#!/usr/bin/env python3
"""
PF2e Data Extractor v8 - Grants Spell + Optimized Traits
========================================================
Basé sur v7, avec ajout d'un champ grants_spell pour les dons
qui accordent des sorts (ex: sorts de focus).
...
Usage:
    python pf2_extract_v8.py              # Télécharge et indexe
    python pf2_extract_v8.py --clean      # Repart de zéro
    python pf2_extract_v8.py --local      # Sans télécharger
    python pf2_extract_v8.py --selftest <base>   # Confronte les dérivations
"""
...
def create_database(entries: List[dict], stats: dict, all_traits: List[Tuple[str, str, str]]):
    ...
    meta = {"created_at": datetime.now().isoformat(), "total": inserted,
            "translated": trans_ct, "stats": json.dumps(stats), "version": "8.0",
...
def main():
    print(f"\n{C.BOLD}{'═' * 60}")
    print("🎲 PF2e Data Extractor v8 - Grants Spell + Optimized Traits")
    print(f"{'═' * 60}{C.RESET}\n")
