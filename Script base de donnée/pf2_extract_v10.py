#!/usr/bin/env python3
"""
PF2e Data Extractor v10 - Étiquettes indexées, @localize résolu
==============================================================
Basé sur v9, dont il garde grants_spell, l'index des traits, la résolution
des libellés de règles et la distinction des pages de journal.

Deux nouveautés, toutes deux mesurées sur la base v8 publiée :

1. Les ÉTIQUETTES (`system.traits.otherTags`) entrent dans `entity_traits`,
   préfixées `tag:`. 560 entrées en portent, 78 étiquettes distinctes. Ce
   sont elles que les règles Foundry désignent — « choisissez une leçon de
   sorcier » s'écrit `item:tag:witch-lesson`, un ikon d'exemplaire
   `item:tag:exemplar-ikon`. Sans index, l'application devait balayer les
   29 000 entrées et relire leur JSON pour retrouver les 19 leçons.
   Le préfixe n'est pas décoratif : `holy`, `thrown` et `unholy` existent à
   la fois comme trait et comme étiquette, et ne désignent pas la même
   chose.

2. Les appels `@localize[PF2E.…]` sont RÉSOLUS dans les descriptions.
   9 283 occurrences, 199 clés distinctes, dans 3 844 entrées — presque
   toutes des capacités de créature (Empoignade, Guérison du vide, Frappe
   réactive…). Le texte n'était nulle part : l'application affichait la clé
   brute, ou au mieux un nom recopié dans son code. Les clés vivent dans
   les fichiers de langue déjà clonés, dans les deux langues.
...
Usage:
    python pf2_extract_v10.py              # Télécharge et indexe
    python pf2_extract_v10.py --clean      # Repart de zéro
    python pf2_extract_v10.py --local      # Sans télécharger
    python pf2_extract_v10.py --selftest <base>   # Confronte les dérivations
"""
...
# Le second usage des mêmes tables : les appels `@localize[…]` glissés DANS le
# texte des descriptions.
#
#   <p>@Localize[PF2E.NPC.Abilities.Glossary.Grab]</p>
#
# Ce n'est pas un champ qui vaut une clé — c'est un trou au milieu d'une phrase,
# que Foundry rebouche à l'affichage. Hors de Foundry, il reste béant : la fiche
# d'un monstre annonce « Empoignade » puis n'en donne pas les conditions. On le
# rebouche ici, une fois pour toutes, plutôt que dans l'application où il
# faudrait un dictionnaire recopié à la main — et qui ne rend qu'un nom.
#
# La langue se déduit du champ qui porte le texte : `description`,
# `description_fr` et tout champ suffixé `_fr` sont français, le reste anglais.
# Si la clé manque dans la langue visée, l'autre langue vaut mieux qu'un trou ;
# si elle manque partout, l'appel reste intact et il est compté.

LOCALIZE_CALL = re.compile(r"@[Ll]ocalize\[([^\]]+)\]")

FR_TEXT_FIELDS = {"description", "description_fr"}


def resolve_localize_text(text: str, primary: Dict[str, str], secondary: Dict[str, str],
                          counter: dict) -> str:
    """Remplace les `@localize[clé]` d'un texte par le texte de la clé.

    Un texte résolu peut en citer un autre : `PF2E.SpecificRule.Shisk.
    PiercingQuills.Note` renvoie un texte qui contient lui-même un appel. D'où
    la reprise, bornée — une clé qui se citerait elle-même ne bouclerait pas.
    """
    missing = []
    ...
    def replace(match):
        ...
        if not value:
            # Compté une seule fois, quel que soit le nombre de reprises.
            missing.append(key)
            return match.group(0)
        counter["résolus"] = counter.get("résolus", 0) + 1
        return value

    for attempt in range(3):
        missing.clear()
        resolved = LOCALIZE_CALL.sub(replace, text)
        if resolved == text:
            break
        text = resolved

    if missing:
        counter["manquants"] = counter.get("manquants", 0) + len(missing)
    return text


def resolve_localize(node, labels: Dict[str, Dict[str, str]], counter: dict, field: str = ""):
    """Parcourt une entrée et résout les `@localize[…]` de tous ses textes."""
    fr = labels.get("fr", {})
    en = labels.get("en", {})
    ...
    if isinstance(node, dict):
        for key, value in node.items():
            if isinstance(value, str):
                if "ocalize[" not in value:
                    continue
                is_fr = key in FR_TEXT_FIELDS or key.endswith("_fr")
                node[key] = resolve_localize_text(
                    value, fr if is_fr else en, en if is_fr else fr, counter
                )
            else:
                resolve_localize(value, labels, counter, key)
    elif isinstance(node, list):
        for value in node:
            resolve_localize(value, labels, counter, field)
...
def extract_traits_from_entry(entry: dict) -> List[str]:
    ...
    if isinstance(traits_obj, dict):
        ...
        # Étiquettes (system.traits.otherTags) — ce que les règles désignent
        # sous `item:tag:…`. Préfixées, car `holy`, `thrown` et `unholy` sont
        # à la fois des traits et des étiquettes sans rapport.
        other_tags = traits_obj.get("otherTags", [])
        if isinstance(other_tags, list):
            for t in other_tags:
                if isinstance(t, str) and t:
                    traits.append(f"tag:{t.lower()}")
...
def create_database(entries: List[dict], stats: dict, all_traits: List[Tuple[str, str, str]]):
    ...
    # Les étiquettes se comptent à part : c'est le nombre que l'application
    # regarde pour savoir si la base sait répondre à `item:tag:…`.
    cur.execute("SELECT COUNT(DISTINCT trait) FROM entity_traits WHERE trait LIKE 'tag:%'")
    unique_tags = cur.fetchone()[0]

    meta = {"created_at": datetime.now().isoformat(), "total": inserted,
            "translated": trans_ct, "stats": json.dumps(stats), "version": "10.0",
    ...
    log(f"  {unique_traits} traits uniques ({unique_tags} étiquettes), "
        f"{traits_inserted} associations", "dim")
...
def selftest(db_path: str) -> int:
    ...
    Usage : python pf2_extract_v10.py --selftest pf2_data/pf2e_v5.db
    """
    path = Path(db_path)
    if not path.exists():
    ...
    # ── étiquettes
    # Sur une base antérieure à v10, ce compte vaut zéro et la vérification des
    # traits ci-dessus signale les 560 entrées concernées : c'est attendu, pas
    # une régression.
    log("Vérification des étiquettes...", "info")
    tagged = conn.execute(
        "SELECT COUNT(DISTINCT trait) FROM entity_traits WHERE trait LIKE 'tag:%'"
    ).fetchone()[0]
    log(f"  {tagged} étiquettes indexées",
        "ok" if tagged else "warn")

    # ── @localize
    # Ce qui reste après résolution : des clés que les fichiers de langue ne
    # portent pas. Les lister vaut mieux que de les découvrir sur la fiche.
    log("Vérification des appels @localize...", "info")
    leftovers = defaultdict(int)
    for row in rows:
        for key in LOCALIZE_CALL.findall(row["data"] or ""):
            leftovers[key.strip()] += 1
    total_left = sum(leftovers.values())
    log(f"  {total_left} appels non résolus, {len(leftovers)} clés distinctes",
        "ok" if not total_left else "warn")
    for key, count in sorted(leftovers.items(), key=lambda kv: -kv[1])[:10]:
        log(f"    {count:5}  {key}", "dim")
...
def main():
    print(f"\n{C.BOLD}{'═' * 60}")
    print("🎲 PF2e Data Extractor v10 - Étiquettes indexées, @localize résolu")
    print(f"{'═' * 60}{C.RESET}\n")
    ...
    # Reboucher les `@localize[…]` — après TOUTES les extractions, pour que les
    # entrées venues du glossaire, des états ou des journaux en profitent aussi.
    log("Résolution des appels @localize...", "info")
    localize_labels = load_rule_labels()
    localize_stats = {}
    for entry in entries:
        resolve_localize(entry, localize_labels, localize_stats)
    log(f"  {localize_stats.get('résolus', 0)} appels résolus, "
        f"{localize_stats.get('manquants', 0)} sans traduction",
        "ok" if localize_stats.get("résolus") else "warn")
    print()
